...
void c_input_number::render() {
	...
	if (std::to_string(format).size() > 0) {
		g_Render->DrawString(pos.x + 10 + offset, pos.y + size.y / 2, color_t(200 + 55.f * animation, 200 + 55.f * animation, 200 + 55.f * animation, alpha),
			render::centered_y, fonts::menu_desc, std::to_string(format).c_str());
	}
	else if (!active)
	{
		g_Render->DrawString(pos.x + 10 + offset, pos.y + size.y / 2, color_t(200 + 55.f * animation, 200 + 55.f * animation, 200 + 55.f * animation, alpha),
