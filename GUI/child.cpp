...
void c_child::render() {
	...
	g_Render->FilledRect(pos.x, pos.y, size.x, size.y, color_t(25, 25, 25, (int)(wnd->get_transparency() * 2.55f)));
	...
	if (should_draw_scroll) {
		...
		g_Render->FilledRect(pos.x + size.x - 9, pos.y + 1, 8, size.y - 2, color_t(35, 35, 35, alpha));
		if (scroll.value > 0) {
			...
			g_Render->_drawList->AddTriangleFilled(
				...
				color_t(140, 140, 140, new_alpha).u32());
		}
		...
		g_Render->FilledRect(pos.x + size.x - 8, pos.y + scroll_pos, 6, size.y - scroll_pos_max,
