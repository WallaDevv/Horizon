...
HRESULT c_font::init_device_objects(IDirect3DDevice9* dev)
{
	...
	font_weigh_fix.push_back(this->weight);
...
void c_font::build_lookup_table()
{
	...
	for (auto i = 0; i <= points; i++)
		lookup_table.emplace_back(
			cos(2.f * D3DX_PI * (i / static_cast<float>(points))),
			sin(2.f * D3DX_PI * (i / static_cast<float>(points)))
		);
}
		...
		if (c != ' ')
		{
			...
			*vertices++ = { { x + 0 - 0.5f, y + 0 - 0.5f, 1.0f, 1.0f }, color, x1, y1 };
			...
			if (num_triangles * 3 > (max_vertices - 6))
			{
				...
				vertices = nullptr;
				this->vertex_buffer->Lock(0, 0, reinterpret_cast<void**>(&vertices), NULL);
				num_triangles = 0L;
			}
		}
...
HRESULT c_font::draw_radial(float x, float y, uint32_t color1, uint32_t color2, const char* text, uint8_t flags)
{
	if (!this->dev)
		return E_FAIL;
	...
	while (*text)
	{
		...
		if (c != ' ')
		{
			...
			const auto center = Vector2D(x - .5f, y - .5f) + Vector2D(w, h) * 0.5f;
			const auto center_vertex = (Vector2D(x1, y1) + Vector2D(x2, y2)) * 0.5f;
			...
			clip.X = std::clamp<uint32_t>(static_cast<uint32_t>(x), minX, maxX);
			clip.Y = std::clamp<uint32_t>(static_cast<uint32_t>(y), minY, maxY);
