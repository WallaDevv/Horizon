...
void Drawing::DrawBox(int x, int y, int w, int h, color_t color)
{
	...
	c_renderer::rect(Vector2D(x, y), Vector2D(w, 1), color);


	c_renderer::rect(Vector2D(x, y + 1), Vector2D(1, h - 1), color);

	c_renderer::rect(Vector2D(x + w - 1, y + 1), Vector2D(1, h - 1), color);

	c_renderer::rect(Vector2D(x, y + h - 1), Vector2D(w - 1, 1), color);
